...
export default function TasksPage() {
  ...
  useEffect(() => {
    async function fetchTasks() {
      try {
        ...
        console.debug('Error fetching tasks', err instanceof Error ? err.message : String(err));
        setError('Failed to load tasks');
      } finally {
        setLoading(false);
      }
  ...
  async function addTask() {
    ...
    try {
      ...
      const data = await res.json();
      const newTask = { ...data.task, status: data.task.status || 'pending' } as Task;
  ...
  }, []); // Remove generateAISummary from dependencies to prevent memory leaks
  ...
  async function saveEditTask() {
    ...
    try {
      ...
      const data = await res.json();
      setTasks((prev) => prev.map(t =>
        t._id === editingTaskId ? { ...data.task, status: data.task.status || t.status } : t
      ));
      cancelEditTask();
    } catch (err: unknown) {
      console.debug('update task failed', err instanceof Error ? err.message : String(err));
...
function DroppableColumn({ column, onDelete, onEdit, generatingSummary }: {
  column: Column;
...
function TaskCard({ task, onDelete, onEdit, generatingSummary }: {
  task: Task;
  onDelete: (id: string) => void;
  onEdit: (task: Task) => void;
  generatingSummary: string | null;
